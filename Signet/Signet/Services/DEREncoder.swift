...
struct DER {
    ...
    static func integerBigNum(_ bytes: [UInt8]) -> Data {
        integer(Data(bytes))
    }
    ...
    static func utf8String(_ str: String) -> Data {
        tag(0x0C, Data(str.utf8))
    }

    static func ia5String(_ str: String) -> Data {
        tag(0x16, Data(str.utf8))
    }
    ...
    static func printableString(_ str: String) -> Data { tag(0x13, Data(str.utf8)) }
...
struct CMSBuilder {
    ...
    static func buildSignedData(
        ...
        let contentTypeAttr = DER.sequence(
            OID.contentType +
            DER.set(OID.pkcs7Data)
        )
        let signingTimeAttr = DER.sequence(
            OID.signingTime +
            DER.set(DER.utcTime(Date()))
        )
        let messageDigestAttr = DER.sequence(
            OID.messageDigest +
            DER.set(DER.octetString(digestOfCD))
        )

        let authAttrsContent = contentTypeAttr + signingTimeAttr + messageDigestAttr
        let authAttrsSet = DER.tag(0x31, authAttrsContent)

        let keyType = SecKeyGetBlockSize(privKey) > 48 ? "RSA" : "EC"
        let sigAlgOID: Data
    ...
    private static func buildIssuerAndSerial(from certData: Data) -> Data {
        ...
        var issuerData = Data()
        if let subjectName = (vals[kSecOIDX509V1SubjectName as String] as? [String: Any])?["value"] as? [[String: Any]] {
